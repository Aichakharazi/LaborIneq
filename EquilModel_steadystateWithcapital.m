...
%% capital
ddelta_K = 0.025;  % capital dep rate
kk = 3;
Inves =  ddelta_K* kk ;
...
xx1 =  zz * ( aalpha * (LY)^((ssp-1)/ssp) + (1-aalpha)* (LM)^((ssp-1)/ssp) )^(ssp/(ssp-1))  ;
margcostgues = total_l_cost / xx1 ;


%margcostcap = rs/((1-alpha_cob)*kk^(-alpha_cob) * xx1^(alpha_cob)) ;

y = kk^(1-alpha_cob) *  (xx1)^(alpha_cob);
...
e_m u_m kappa kk alpha_cob xx1 Inves rks;
